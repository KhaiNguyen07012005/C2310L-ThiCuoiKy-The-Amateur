...
<body>
    ...
    <main>
        ...
        <div class="table-data">
            <div class="order">
                <div class="head">
                    <h3>Báo Cáo Tài Chính AutoRent</h3>
                    <button onclick="openModal()" class="btn-pdf">Tải PDF</button>
                    <a href="{{ route('dashboard_home_', ['range' => 'week']) }}" class="btn btn-primary">Theo Tuần</a>
                    <a href="{{ route('dashboard_home_', ['range' => 'month']) }}" class="btn btn-primary">Theo Tháng</a>
                    <a href="{{ route('dashboard_home_', ['range' => 'year']) }}" class="btn btn-primary">Theo Năm</a>
                </div>
                <table>
                    <thead>
                        @php
            ...
            <div class="laporan_form">
                <div class="">
                    <form action="{{ route('pdf_keuangan', ['range' => 'week']) }}" method="post">
                        @csrf
                        <button class="btn-pdf" type="submit">Báo Cáo Tuần</button>
                    </form>
                </div>
                <div>
                    <form action="{{ route('pdf_keuangan', ['range' => 'month']) }}" method="post">
                        @csrf
                        <button class="btn-pdf" type="submit">Báo Cáo Tháng</button>
                    </form>
                </div>
                <div>
                    <form action="{{ route('pdf_keuangan', ['range' => 'year']) }}" method="post">
                        @csrf
                        <button class="btn-pdf" type="submit">Báo Cáo Năm</button>
                    </form>
                </div>
            </div>
        </div>
